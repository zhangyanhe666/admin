...
class TimRestAPI extends TimRestInterface
{
	...
	public function generate_user_sig($identifier, $expiry_after, $protected_key_path, $tool_path)
	{
		...
		isset($out[0]) && $this->usersig = $out[0];
		return $out;
	}
    ...
    public function openim_dirty_words()
    {
            $ret = $this->api("openim_dirty_words", "get", $this->identifier, $this->usersig,'{}');
    $ret = json_decode($ret, true);
            return $ret;
    }
    public function ToolApi($control, $action,$data="{}"){
        $ret = $this->api($control,$action, $this->identifier, $this->usersig,$data,false);
        $ret = json_decode($ret, true);
        return $ret;
    }
};
