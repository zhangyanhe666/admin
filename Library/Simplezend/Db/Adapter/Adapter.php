...
class Adapter implements AdapterInterface, Profiler\ProfilerAwareInterface
{
    ...
    public static $adapterList  =   array();
    /**
    ...
    public function __construct($driver, Platform\PlatformInterface $platform = null, ResultSet\ResultSetInterface $queryResultPrototype = null, Profiler\ProfilerInterface $profiler = null)
    {
        ...
        if (is_array($driver)) {
            ...
            throw new Exception\InvalidArgumentException(
                'The supplied or instantiated driver object does not implement Library\Db\Adapter\Driver\DriverInterface'
            );
        }
    ...
    public static function getInstance($driver, Platform\PlatformInterface $platform = null, ResultSet\ResultSetInterface $queryResultPrototype = null, Profiler\ProfilerInterface $profiler = null){
        $k  =   md5(json_encode($driver),true);
        if(!isset(self::$adapterList[$k])){
            self::$adapterList[$k]    =   new Adapter($driver,$platform,$queryResultPrototype,$profiler);
        }
        return self::$adapterList[$k];
    }
    /**
     * @param Profiler\ProfilerInterface $profiler
     * @return Adapter
