...
class ChildMenu extends SysModel
{
    ...
    public function getMenu(){
        return $this->getItem($this->getService('router')->getMenuId());
    }
    //获取用户所有子菜单
    public function getChildMenus($id){
        ...
        if($this->getService('Model\AdminGroup')->isSuperAdmin($gid)){
            ...
            $cmenus =   $this->where(array('is_show'=>0,'b.gid'=>$gid))->join(array('b'=>'sys_group_auth'),'sys_childmenu.id=b.menu_id',array())->order(array('sys_childmenu.sort','sys_childmenu.id'))->getAll()->toArray();
        }
        return $cmenus;
    }
    //获取指定父id的子菜单
